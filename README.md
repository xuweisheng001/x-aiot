...
> 定位：**技术预研原型**，不是生产系统。所有数字来自本机单机实测，报数字先报口径
> （docs/loadtest-report.md，2026-09-19 已回填：5000 连接零假成功、3000 台风暴 33s 回归、
> 3 万条泄洪对账零丢失 3528 msg/s，并实测推翻了「加 flusher 能提吞吐」的原有假设）。
...
`.github/workflows/ci.yml` 五个作业：构建与格式、全量竞态单测、**安全守卫**、订阅隔离依赖扫描、集成测试（带服务容器）。

安全守卫单列一个作业并按名字点出用例，因为这些失败意味着某条安全边界被拆掉了，
不该淹在全量输出里：指令白名单与授权、Agent 只读边界、诊断包字段白名单、租户隔离、
调度器不重复下发、隐私开关三态、材料码不带出参数、OTA 熔断与灰度、单实例锁键不碰撞。
每个步骤名都带对应的事故编号，可回查 `docs/incident-premortem-bl*.md`。
