...
| job-svc | 8091 | BL4 加工记录与反哺（技术方案 §8）。durable consumer `job` 订 `iot.up.event.*`，只处理 JOB_START/DONE/FAIL/PAUSE；**opt-in fail-closed**：影子 reported.`job_feedback_optin` 为 true 才落库，false / 缺失 / Redis 出错一律丢弃并计数 dropped_optin_*；字段白名单 job_id UUID36、material_id/param_profile_id `^[A-Za-z0-9_-]{1,32}$`、params_hash 64 hex（不符丢弃 dropped_bad_fields）；JOB_START → job_record，终态 → finished_at/outcome/duration_s（START 丢失补插计 late_start）；`POST /api/v1/jobs/{job_id}/feedback {rating}`（头 `X-Device-Sn` 须与 job_record.sn 一致；job_record 未到 → 202 暂存 job_feedback_pending，到达后同事务合并）；撤回删除以 **desired**.job_feedback_optin=false 为触发（`IOT_JOB_PURGE_INTERVAL` 默认 10m）；**opt-in 对账（INC-4-18）**：每 `IOT_JOB_OPTIN_RECONCILE_INTERVAL`（1h）或 `POST /internal/job/optin/run` 取 job_record distinct sn 逐个读影子，纯函数 `ViolatingSNs`（false / 缺失 → 违规，Redis 出错既不违规也不删只计 optin_shadow_errors）→ slog.Error + optin_violations + 复用 PurgeSN 删除并计 optin_purged；`GET /metrics` |
...
| probe | 8096 | 安全事件合成探针（BL1 预推演 INC-10 P0 根治项）。每 `IOT_PROBE_INTERVAL`（默认 10m，**必须大于 alarm squelch 5 min**，否则第二发被聚合吞掉会误报漏告警）经真实链路打一发 `IOT_PROBE_CODE`（默认 FLAME_DETECTED），量 event_ts → alarm.notified_at 的端到端时间：≤ `IOT_PROBE_SLO`（3s）为 ok，超了 slow（S2），`IOT_PROBE_TIMEOUT`（默认 max(2×SLO,10s)）内没等到 notified_at 即 **missing（S1）并 ERROR**。落库但未推送同样算 missing——用户收不到就是漏告警。发射路径 `-mode`/`IOT_PROBE_MODE`：`mqtt`（默认，经 conn-gate→EMQX→bridge→JetStream，地址 `IOT_PROBE_MQTT_ADDR` 默认 127.0.0.1:1884，每轮重新拨号以覆盖接入面）或 `jetstream`（绕开接入面定位后半段）。探针**关闭自己造的告警**（acked→closed，closed_by=probe），否则每 10 分钟给运营堆一条合成火警造成告警疲劳；`IOT_PROBE_CLEANUP=false` 可关。`-once` 单发并以退出码表达结果（missing → 1，供 cron/CI 判 S1）；`POST /internal/probe/run` 手动触发；`GET /metrics`（probe_runs/ok/slow/missing/errors/cleaned + 延迟分位）。探针设备需先在 iot_shard.device 里 activated |
