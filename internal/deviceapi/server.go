...
func (s *Server) postCmd(w http.ResponseWriter, r *http.Request) {
	...
	rec := AuditRecord{CmdID: cmdID, SN: sn, Action: req.Action, Params: req.Params, Operator: req.Operator,
		Source: req.Source, Result: "dispatched", CreatedAt: s.now().UTC()}
	body, _ := json.Marshal(rec)
	actx, cancel := context.WithTimeout(r.Context(), 5*time.Second)
	defer cancel()
	if _, err := s.JS.Publish(actx, envelope.SubjectCmdAudit, body); err != nil {
		slog.Error("publish audit", "cmd_id", cmdID, "err", err) // 指令已下发，审计尽力而为
	}
